...
void encoder_speed_()
{
    ...
    encoder_speed.left = (encoder_count.left * encoder_direction.left * 60) / (encoder_setup.PULSES_PER_REV * encoder_setup.FRAME_TIME_MS);
    // right encoder speed calculation (rpm)
    encoder_speed.right = (encoder_count.right * encoder_direction.right * 60) / (encoder_setup.PULSES_PER_REV  * encoder_setup.FRAME_TIME_MS);
}
